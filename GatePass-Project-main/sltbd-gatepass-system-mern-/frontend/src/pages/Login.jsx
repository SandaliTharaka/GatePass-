...
const Login = () => {
  ...
  const handleAzureLogin = async () => {
    ...
    try {
      ...
      if (data.token) {
        console.log("Azure login data:", data);
        localStorage.setItem("user", JSON.stringify(data));
        const roleToStore = String(data.roles).trim();
        localStorage.setItem("role", roleToStore);
        const token = String(data.token).trim();
        localStorage.setItem("token", token);
          ...
          <div className="w-full md:w-5/12 bg-gradient-to-br from-blue-900 to-indigo-900 p-8 flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-4 text-white">
                <h2 className="text-2xl font-bold tracking-tight">Welcome to Gate Pass Portal text 1</h2>
