...
const Login = () => {
  ...
  useEffect(() => {
    ...
    // Clear all MSAL-related session storage items
    try {
      const keysToRemove = [];
    ...
    return () => {
      // Cleanup on unmount - only if we're not in the middle of Azure login
  ...
  const handleAzureLogin = async () => {
    ...
    // Clear any stale MSAL data before starting
    try {
      // Clear all MSAL-related session storage items
      const keysToRemove = [];
      for (let i = 0; i < sessionStorage.length; i++) {
    ...
    // Store loading state in sessionStorage so we can check it on return
    sessionStorage.setItem('azureLoginInProgress', 'true');
    
    try {
      await authService.azureLogin();
      // Note: This will redirect to Microsoft, so code below won't execute
      // The callback page will handle the response
    } catch (error) {
      console.error("Azure login error:", error);
      showToast(error.message || "Azure login failed. Please try again.", "error");
      setIsAzureLoading(false);
      sessionStorage.removeItem('azureLoginInProgress');
    }
  };
