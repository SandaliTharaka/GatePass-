...
const generateDESCRIPTIONDetailsPDF = (fullRequest) => {
  return generateGatePassPdf(fullRequest, {
    title: "SLT Gate Pass - Verification Report",
    subtitle: "Decorated request report with all available details",
    fileNamePrefix: "SLT_GatePass_Verification",
  });
};
...
const fetchReceiverDetails = async (serviceNo) => {
  ...
  try {
    ...
    return {
      ...
      serviceNo: employee.employeeNo || serviceNo,
...
const fetchOfficerData = async (status) => {
  ...
  if (execServiceNo) {
    try {
      executiveOfficerData = await getCachedUser(execServiceNo, searchUserByServiceNo);
    } catch {}
  }
  if (verifyServiceNo) {
    try {
      verifyOfficerData = await getCachedUser(verifyServiceNo, searchUserByServiceNo);
    } catch {}
  }
...
const RequestDetailsModal = ({
  ...
  const printReport = (request, transporterDetails, loadingStaff) => {
    // Use the existing PDF print flow from gatePassPdf.js.
    // This avoids fragile iframe-based printing that can trigger
    // "Unsafe attempt to load URL ... from frame" browser errors.
    try {
      printGatePassPdf(request, {
        title: "SLT Gate Pass - Verification Report",
        subtitle: "Complete request, transport, and approval summary",
        senderFallback: userDetails,
        transporterDetails,
